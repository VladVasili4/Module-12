...
class TournamentTest(unittest.TestCase):
    ...
    def test_Tournament4(self):
        try:
            self.t4 = Tournament(90, self.r1, self.r4)
            self.all_results = self.t4.start()
            self.assertTrue(self.all_results[2] == 'Усэйн')
        except:
            AssertionError, TournamentTest.all_results.append(f' {AssertionError} :{self.all_results} '
                                                              f'Должно быть наоборот !!!')
    def tearDown(self):
        pass
    @classmethod
